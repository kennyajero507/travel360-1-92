...
const SignIn = () => {
  ...
  useEffect(() => {
    if (!authLoading && session && userProfile) {
      console.log('[SignIn] User is logged in, redirecting to:', from);
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      console.log('[SignIn] Attempting signin for:', formData.email);
      const success = await login(formData.email, formData.password);
      
      if (success) {
        console.log('[SignIn] Signin successful');
        toast.success("Welcome back!");
        // Navigation will be handled by useEffect when session updates
      } else {
        console.log('[SignIn] Signin failed');
