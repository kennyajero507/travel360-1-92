...
export const createInquiry = async (inquiryData: InquiryInsertData): Promise<InquiryData> => {
  ...
  try {
    ...
    // Get user profile to ensure they belong to an organization
    ...
    if (!profile?.org_id && profile?.role !== 'system_admin') {
      throw new Error("You must belong to an organization to create inquiries");
    }

    // Prepare the data for insertion
    const insertData = {
      id: inquiryData.id,
      enquiry_number: 'TEMP', // Will be auto-generated by database trigger
  ...
  try {
    ...
    if (error) {
      ...
      if (error.message.includes('row-level security') || error.message.includes('policy')) {
        toast.error("You don't have permission to update this inquiry");
      } else {
...
export const getInquiryById = async (id: string): Promise<InquiryData | null> => {
  ...
  try {
    ...
    if (error) {
      ...
      if (error.message.includes('row-level security') || error.message.includes('policy')) {
        toast.error("You don't have permission to view this inquiry");
      } else {
...
export const deleteInquiry = async (id: string): Promise<boolean> => {
  ...
  try {
    ...
    if (error) {
      ...
      if (error.message.includes('row-level security') || error.message.includes('policy')) {
        toast.error("You don't have permission to delete this inquiry");
      } else {
