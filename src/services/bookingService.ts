...
export const createBookingFromQuote = async (quoteId: string, hotelId: string): Promise<Booking> => {
  try {
    ...
    // Create booking data - booking_reference will be auto-generated by trigger
    const bookingData = {
      client: quote.client,
      ...
      quote_id: quoteId,
      notes: quote.notes || '',
      booking_reference: '' // Will be auto-generated by trigger, but required by TypeScript
    };
...
// Legacy create booking function for backward compatibility
export const createBooking = async (bookingData: any): Promise<Booking> => {
  try {
    // If called with old interface, extract quote and hotel IDs
    if (bookingData.quoteId && bookingData.hotelId) {
      return await createBookingFromQuote(bookingData.quoteId, bookingData.hotelId);
    }
